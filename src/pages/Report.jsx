...
import { loadScuolaData } from '../services/scuola.js';
import {
  buildCsv,
  buildXlsxBlob,
...
const downloadCsv = (rows, key) => exportCsv(rows, key, csvColumns[key]);
const downloadPdf = (rows, key) => exportPdf(rows, key, csvColumns[key]);
const downloadXlsx = (rows, key) => exportXlsx(rows, key, csvColumns[key]);
...
export default function Report() {
  ...
  // Statistiche di accesso: dati personali, visibili solo ad admin e presidente.
  const canViewAnalytics = role === 'admin' || role === 'presidente';
  ...
  const [scuolaData, setScuolaData] = useState(null);
  const [scuolaError, setScuolaError] = useState('');
  const refreshScuolaData = useCallback(async () => {
    try {
      const remote = await loadScuolaData();
      setScuolaError('');
      setScuolaData(remote ? { ...remote.document, updatedAt: remote.updatedAt } : null);
    } catch (loadError) {
      console.error('[Report] Impossibile leggere i dati della scuola:', loadError);
      setScuolaError('Impossibile caricare i dati della scuola dal server.');
    }
  }, []);
  ...
  }, [isAuthenticated, canViewAnalytics, visitsRange]);
  ...
  }, [isAuthenticated, canViewAnalytics, ACTIVE_REFRESH_MS, activeRange]);
  ...
  async function handleMembersSummaryExport(format = 'csv') {
    ...
    if (format === 'pdf') {
      ...
      const blob = buildXlsxBlob(sociSummaryColumns, membersSummary);
  ...
  async function handleMembersFullExport(format = 'csv') {
    ...
    if (format === 'pdf') {
      ...
      const blob = buildXlsxBlob(csvColumns.soci_full, rows);
  ...
  async function handleMembersStatusExport(status, format = 'csv') {
    ...
    if (format === 'pdf') {
      ...
      const blob = buildXlsxBlob(csvColumns.soci_full, rowsWithTotal);
  ...
  async function handleActivityExport(format = 'csv') {
    ...
    if (format === 'pdf') {
      ...
      const blob = buildXlsxBlob(csvColumns.activity_logs, activityRows);
  ...
  async function handleVisitsExport(format = 'csv') {
    ...
    if (format === 'pdf') {
      ...
      const blob = buildXlsxBlob(csvColumns.analytics_visits, visitsRows);
  ...
  async function handleActiveUsersExport(format = 'csv') {
    ...
    if (format === 'pdf') {
      ...
      const blob = buildXlsxBlob(csvColumns.access_events, activeRows);
  ...
  return (
    <section className="page-grid">
      ...
      <article className="card">
        ...
        {scuolaError && <p style={{ color: '#c92a2a' }}>{scuolaError}</p>}
        {scuolaSummary ? (
